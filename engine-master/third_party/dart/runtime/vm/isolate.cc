...
MessageHandler::MessageStatus IsolateMessageHandler::HandleMessage(
  ...
  if (!message->IsOOB() && (message->dest_port() != Message::kIllegalPort)) {
    //调用Dart层的_lookupHandler函数，该函数在isolate_patch.dart中
...
#endif
    //调用Dart层的_handleMessage函数，该函数在isolate_patch.dart中
    const Object& result =
...
#if !defined(DART_PRECOMPILED_RUNTIME)
      native_callback_trampolines_(),//非AOT编译
#endif
#if !defined(PRODUCT)//非生产环境
...
#undef ISOLATE_METRIC_INIT
  ...
  //1、创建Isolate对应的堆空间，在该堆空间中，存在对象的分配，垃圾回收等。
  Heap::Init(result,
             is_vm_isolate
